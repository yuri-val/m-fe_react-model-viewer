...
`;
/** 执行解压操作 */
export async function inflate(modelFile) {
 ...
 * @returns
 */
export async function deflate(modelFile) {
    let buffer = await readFileAsArrayBufferAsync(modelFile);
    let intArray = new Uint8Array(buffer);
    let deflatedFile;
    try {
 ...
 * @returns
 */
export async function deflateByZip(modelFile) {
    // 读取文件内容
    let buffer = await readFileAsArrayBufferAsync(modelFile);
    let intArray = new Uint8Array(buffer);
    const zip = new JSZip();
 ...
 * @returns
 */
export async function unzip(modelFile) {
    let buffer = await readFileAsArrayBufferAsync(modelFile);
    let intArray = new Uint8Array(buffer);
    // 执行解压操作
    let unzipFile;
