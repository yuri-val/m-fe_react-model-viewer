...
import { deflate, getFileObjFromModelSrc, isSupportOcctLoader, isSupportThreejsLoader, loadMeshWithRetry, } from '../../utils';
import { adjustGeometry, calcTopology, cookMeshMaterial } from '../core';
import { ObjectSnapshotGenerator } from '../generator';
import { OrbitControls, OrbitControlsGizmo } from '../scenery/controls';
import { ThreeDryRenderer } from './ThreeDryRenderer';
import { ThreeRendererContext } from './ThreeRendererContext';
const fudge = 1.0;
export class ThreeRenderer extends ThreeDryRenderer {
    constructor(
    // viewerProps 相对是常量
    ...
    async loadModel() {
        ...
        try {
            ...
            if (props.mesh) {
                ...
                const modelFile = await getFileObjFromModelSrc(Object.assign(Object.assign({}, props), { compressType: this.viewerProps.compressType }));
                ...
                if (!(isSupportThreejsLoader(props.type) ||
                    isSupportOcctLoader(props.type) ||
                    props.renderOptions.withMesh)) {
                    return;
                }
                // 进行模型实际加载，注意，不需要转化为
                ({ mesh, group } = await loadMeshWithRetry(modelFile || props.src, props.type, {
                    toGltf: false,
                    originSrc: props.src,
                    props,
                }));
    ...
    async renderMesh(mesh) {
        ...
        const material = cookMeshMaterial(context.withClipping, this.getViewerState().modelColor);
    ...
    setupEquipments() {
        ...
        if (this.context.camera && this.$dom) {
            ...
            this.context.controlsGizmo = new OrbitControlsGizmo(this.context.orbitControls, {
                size: 100,
                padding: 4,
            });
